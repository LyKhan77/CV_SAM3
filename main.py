...
async def lifespan(app: FastAPI):
    ...
    # Load the model and store it in the app_state
...
async def set_stream_url(request: StreamRequest):
    """Sets the RTSP stream URL."""
    print(f"Received new stream URL: {request.url}")
    # Clear old URL to force re-connection in the loop
    app_state["rtsp_url"] = None 
    await asyncio.sleep(0.1) # Give time for the loop to notice
...
async def set_prompt(request: PromptRequest):
    """Sets the object detection prompt."""
    print(f"Received new prompt: {request.object_name}")
...
async def set_limit(request: LimitRequest):
    """Sets the maximum detection limit."""
    print(f"Received new limit: {request.value}")
...
async def set_sound_toggle(request: SoundRequest):
    """Enables or disables the notification sound."""
    print(f"Received sound setting: {request.enabled}")
